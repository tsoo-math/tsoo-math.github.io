...
\begin{eqnarray*}
p(t) &=& \mathbb{P}(O_t)  \\
&=& \mathbb{P}( \text{the machine is working at time $t$})
\end{eqnarray*}
$$
...
* By considering  the event $\{W_0 \leq t\}$, and conditioning on $X_1$  we have
$$
\begin{eqnarray*}
...
*  We are interested in the cumulative **reward process** given by
...
*  if $X_1$ is the first replacement, it has cumulative distribution given by $$\bar{F}(x) = F(x) \mathbf{1}[x <L] +\mathbf{1}[x \geq L]$$
and
...
* Suppose the chain starts at state $s$, and consider the renewal process given by inter-arrival times $Y_i$ corresponding to how the long chain spends at the other states after leaving $s$, before returning to $s$.    
...
##  Version: `r format(Sys.time(), '%d %B %Y')`
  * [Rmd Source](https://tsoo-math.github.io/ucl/intro-renewalthree-slides.Rmd)
