...
```{exercise, name="Borel-Cantelli"}


*  Let $P$ be a transition matrix for a Markov chain.  Show that if 

$$ \sum_{n =1} ^{\infty} p_{ss}(n) < \infty$$
  then the state $s$ is transient.

*  Consider a sequence of  coin-flips $(X_n)_{n\in \mathbb{Z}^+}$ where $p_n = \frac{1}{n^2}$ and $\mathbb{P}(X_n = 1) = p_n$.    Show that there is a *random* time $N$ such that $\mathbb{P}(N < \infty) =1$ and for all $n > N$, we have $X_n =0$.  
...
*  Let $X$ be a Markov chain with transition matrix $P$; assume that $X$ is started at the state $s$.  Let 
$A_n =\{ X_n = s\}$.   Then applying the first Borel-Cantelli lemma to the sequence of sets $A_n$, we obtain the desired result.

*  Let $A_n = \{ X_n=1 \}$.  Again, the result is immediate from the first Borel-Cantelli lemma.
```
