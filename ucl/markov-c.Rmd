...
Let $U_0, U_1, \ldots, V_0, V_1, \ldots$ be independent random variables that are uniformly distributed in $[0,1]$ and independent of $X_0$ and $Y_0$.   Set $X'_0 = X_0$ and  $X_n' = \phi(X_{n-1}', U_{n-1})$.  Similarly,  set  $Z_0' = Y_0$ and $Z_n' = \phi(Z_{n-1}', V_{n-1})$ Thus 
...
$$Y'= \Big(Z_0', \phi(Z_0',V_0), \phi(Z_1', V_1), \phi(Z_2', V_2), \dots, \phi(Z_T', U_T) = \phi(X_T', U_T),  \phi(X'_{T+1}, U_{T+1}), \ldots \Big)$$
...
Let $P$ be an aperiodic irreducible transition matrix on a finite state space $S$.   Let $X$ be a Markov chain with transition matrix $P$.   Fix a state $s\in S$, and  set 
