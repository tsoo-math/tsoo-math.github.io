...
$$ 
\begin{eqnarray*}
&& G_m(i)= \Big( X_0=i, X_1=\phi(i,U_0), X_2= \phi(X_1, U_1)  \\
...
*  With this coupling, at this time $T$ and after, the starting point of the chain is in some sense forgotten, since regardless of the initial values, it ends up with the same value values $g_m(1)$ for $m \geq T$.  Although, in general $g_T \not = g_{T+1}$.
...
*  General idea is if we run an Markov chain from time $-\infty$ to time $0$, then 
if it is aperiodic and irreducible, by time $0$ it will have reached the stationary distribution.

* Consider now
$$ 
\begin{eqnarray*}
&& G_m(i)= \Big( X_{-m}=i, X_{-m+1}=\phi(i,U_{-m}),  \\
&& X_{-m+2}= \phi(X_{-m+1}, U_{-m+1})\\
&&, \ldots X_0= \phi(X_{-1}, U_{-1}) \Big)
\end{eqnarray*}
$$
...
* This is the [price](https://www.youtube.com/watch?v=oYE_EibkEkg) we pay for the magic of  *exact* sampling.
...
Here the *rome* function replaces the *london* function.  Notice we need to keep track and re-use 
the uniform random variables generated.
