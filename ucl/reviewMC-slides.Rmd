---
title: "Some review of finite state Markov chains"
...
*  We say that $X$ is a (homogeneous) **Markov chain** if for all $n \geq 0$, we have
$$
\begin{eqnarray*}
&& \mathbb{P}(X_{n+1}  =j \ | \ X_n=i, X_{n-1}=a_{n-1}, \ldots, X_0 = a_{0}) \\
...
*  We call the matrix $P$ given by entries $$P_{ij} = p_{ij} =  \mathbb{P} (X_1 = j \ | \  X_0 =i)$$ 
the **transition matrix** for the chain $X$.  
...
## continued
  ...
  $$
  \begin{eqnarray*}
&& \mathbb{P} (X_{n} =j | X_0=i) \\
&=& \sum_k  \mathbb{P} (X_{n} =j, X_{n-1} = k |  X_0=i) \\
...
* Recall by the handshaking lemma, 
$$\sum_{i \in V} \deg(i) = 2|E|,$$ since each edge contributes value two to the sum.  
* Hence $\pi$ is a probability measure on $V$.  Since $(i,j) \in E$ if and only if $(j, i) \in E$,an easy calculation gives that 
...
##  Version: `r format(Sys.time(), '%d %B %Y')`
  * [Rmd Source](https://tsoo-math.github.io/ucl/reviewMC-slides.Rmd)
